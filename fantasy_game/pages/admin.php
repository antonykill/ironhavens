...
try {
    ...
    $is_admin = isset($result['is_admin']) && $result['is_admin'] == 1;
} catch (PDOException $e) {
    // Gestisce l'errore
    error_log("Errore nell'accesso al database: " . $e->getMessage());
}

// Se l'utente non è un amministratore, reindirizza alla pagina principale con un messaggio
if (!$is_admin) {
    // Salva un messaggio di errore in sessione
    $_SESSION['admin_error'] = "Accesso non autorizzato. Richiesti privilegi di amministratore.";
    header('Location: index.php');
    exit;
}

// Se c'era un messaggio di errore nella sessione, rimuovilo
